...
Entity.prototype._docToEntity = function (doc, done) {
  ...
  function load(field, type, machineName) {
  ...
  function subload(field, value) {
    return function (next) {
      ...
      value.forEach(function (item) {
        queue.push(load(field, item.type, item.machineName));
      });
  ...
  Entity.super_.prototype._docToEntity.call(this, doc, function (err) {
    ...
    for (var field in me._fieldData) {
      ...
      /*
      } else if (me._fieldData[field].machineName) {
        queue.push(load(
          field,
