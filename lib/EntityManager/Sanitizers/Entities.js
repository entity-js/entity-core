...
var async = require('async'),
    ...
    EInvalidValue = loader(
      'Entity/Sanitizers/Errors/EInvalidValue'
    ),
...
module.exports = function sanitizeEntities (orig, value, options, next) {
  ...
  if (options.type === undefined) {
    return next(new EMissingTypeConfig());
  }
  ...
  function loadEntity(idx, item) {
    return function (nxt) {
      ...
      me.core.entityManager.load(options.type, item, function (err, entity) {
        if (err) {
          return nxt(err);
        }
